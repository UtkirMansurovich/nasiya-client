        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-3.5 sm:p-5 flex flex-col h-full">
            <div className="flex items-center justify-between mb-4 sm:mb-5">
                <div className="flex items-center gap-2 sm:gap-3">
                    ...
                    <h4 className="font-bold text-gray-800 text-sm sm:text-base">Bugungi to'lovlar</h4>
                </div>
                <button className="text-xs sm:text-sm font-semibold text-(--secondary) hover:text-blue-700 transition-colors">Barchasi</button>
            </div>
                ...
                {dataDashboardPayments?.map((item: IDashboardPayments, index: number) => (
                    ...
                    >
                        <div className="flex items-center gap-2 sm:gap-4 flex-1 min-w-0">
                            <div className={`w-10 h-10 sm:w-12 sm:h-12 rounded-full ${avatarColors[index % avatarColors.length]} font-bold flex items-center justify-center shrink-0 text-sm sm:text-lg shadow-sm group-hover:scale-105 transition-transform`}>
                            ...
                            <div className="flex flex-col flex-1 min-w-0">
                                ...
                                <div className="flex flex-wrap items-center gap-1 sm:gap-2 text-[10px] sm:text-[13px] text-gray-500 mt-1">
                                    <p className="bg-gray-100/80 px-1.5 sm:px-2 py-0.5 rounded-md text-gray-600 border border-gray-200/60 shadow-xs truncate max-w-[70px] sm:max-w-none">{item?.product}</p>
                                    <span className="hidden sm:inline text-gray-300">—</span>
                                    <p className={item?.status === "Faol" ? "text-emerald-600 font-medium bg-emerald-50 px-1.5 sm:px-2 py-0.5 rounded-md" : "text-rose-600 font-medium bg-rose-50 px-1.5 sm:px-2 py-0.5 rounded-md"}>
                                        {item?.status}
                                    </p>
                                </div>
                            </div>
