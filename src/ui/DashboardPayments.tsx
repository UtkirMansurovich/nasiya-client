        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-5 flex flex-col h-full">
            <div className="flex items-center justify-between mb-5">
                <div className="flex items-center gap-3">
                    <div className="w-8 h-8 rounded-lg bg-(--secondary-hover) text-(--secondary) flex items-center justify-center">
                    ...
                    <h4 className="font-bold text-gray-800 text-base">Bugungi to'lovlar</h4>
                </div>
                <button className="text-sm font-semibold text-(--secondary) hover:text-blue-700 transition-colors">Barchasi</button>
            </div>
            <div className="flex flex-col gap-2">
                {dataDashboardPayments?.map((item: IDashboardPayments, index: number) => (
                    <div
                        key={item?.id}
                        className="p-3 flex items-center justify-between hover:bg-gray-50 transition-all duration-200 rounded-xl border border-transparent hover:border-gray-200 group"
                    >
                        <div className="flex items-center gap-4">
                            <div className={`w-12 h-12 rounded-full ${avatarColors[index % avatarColors.length]} font-bold flex items-center justify-center shrink-0 text-lg shadow-sm group-hover:scale-105 transition-transform`}>
                            ...
                            <div className="flex flex-col">
                                ...
                                <div className="flex items-center text-[13px] text-gray-500 mt-1">
                                    ...
                                    <p className={item?.status === "Faol" ? "text-emerald-600 font-medium bg-emerald-50 px-2 py-0.5 rounded-md" : "text-rose-600 font-medium bg-rose-50 px-2 py-0.5 rounded-md"}>
                                        {item?.status}
                                    </p>
                                </div>
                            </div>
