...
import { CreditCardOutlined, CustomerServiceOutlined, DashboardOutlined, DollarOutlined, LeftOutlined, TableOutlined, PlusOutlined } from "@ant-design/icons";
...
export const SideBar: FC = (): JSX.Element => {
    ...
    return (
        <>
            {/* Desktop Sidebar */}
            <div className={`hidden md:flex ${sidebar.isCollapsed === "true" ? "w-[260px]" : "w-[80px]"} bg-white transition-all duration-300 ease-in-out row-start-2 row-end-3 col-start-1 col-end-2 shadow-(--shadow-right) z-10 flex-col justify-between border-r border-gray-100`}>
            ...
            <div className="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-gray-100 flex items-center justify-around px-2 py-2 pb-[calc(env(safe-area-inset-bottom)+0.5rem)] z-50 shadow-[0_-5px_20px_-5px_rgba(0,0,0,0.05)]">
                <Link to="/dashboard" className={`flex flex-col items-center justify-center w-14 h-12 gap-1 rounded-xl transition-colors ${location.pathname.includes('dashboard') ? 'text-(--secondary)' : 'text-gray-400 hover:text-gray-600'}`}>
                    <DashboardOutlined className="text-[20px]" />
                    <span className="text-[10px] font-bold">Bosh</span>
                </Link>

                <Link to="/customers" className={`flex flex-col items-center justify-center w-14 h-12 gap-1 rounded-xl transition-colors ${location.pathname.includes('customers') ? 'text-(--secondary)' : 'text-gray-400 hover:text-gray-600'}`}>
                ...
                <Link to="/loans" className={`flex flex-col items-center justify-center w-14 h-12 gap-1 rounded-xl transition-colors ${location.pathname.includes('loans') ? 'text-(--secondary)' : 'text-gray-400 hover:text-gray-600'}`}>
                    <CreditCardOutlined className="text-[20px]" />
                    <span className="text-[10px] font-bold">Nasiya</span>
                </Link>

                <Link to="/reports" className={`flex flex-col items-center justify-center w-14 h-12 gap-1 rounded-xl transition-colors ${location.pathname.includes('reports') ? 'text-(--secondary)' : 'text-gray-400 hover:text-gray-600'}`}>
